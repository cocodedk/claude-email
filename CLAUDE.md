...
Email-driven wrapper for the Claude Code CLI. Polls `agent@example.com` via IMAP, verifies that commands come exclusively from `user@example.com` (GPG signature or shared secret), executes them via `claude --print`, and replies via SMTP. Runs as a hardened systemd service.
