# Consolidated v2 Plan — Fix Dirty-Block & Branch Reuse
...
**Version:** v2 (timestamp 2026-05-17)
...
| `src/task_queue.py` | Import `public_row` from `task_row_redact`; `enqueue()` gains `branch_name` and `mutates_repo`. |
| `src/git_ops.py` | Add `branch_exists()`, `checkout_existing_branch()`, `is_valid_task_branch()`. |
...
| `chat/project_tools.py` | `enqueue_task_tool` accepts `mutates_repo`; auto-classifies when None; returns accurate `planned_branch` (empty for read-only). |
...
Total tasks: 14. Expected new tests: ~50. Expected final count: ~1262 passing (baseline 1212 + ~50).
...
Expected: all pass; total **1214 passed** (1212 + 2 new); no file >200 lines.
...
Expected: **1224 passed** (1220 + 4 new).
...
Decision order:
  ...
  3. After stripping polite prefixes, first token
     is a read-only verb / interrogative          → False
...
def classify_mutation(body: str) -> bool | None:
    ...
    first = stripped_tokens[0] if stripped_tokens else ""
...
Expected: all PASS; total **~1252 passed** (1224 + ~28 new — exact count depends on parametrize expansion).
...
Expected: all PASS; total **~1255 passed** (1252 + 3 new); `src/task_queue.py` under 200 lines.
...
Expected: total **~1260 passed**; `src/task_queue.py` still under 200 lines (≈195).
...
Expected: all PASS; total **~1264 passed** (1262 + 2 new); no file >200 lines.
...
### Step 1: Add helpers to `src/git_ops.py`

- [ ] **Step 1a: Failing tests for the new git_ops helpers**
...
- [ ] **Step 1c: Add helpers to `src/git_ops.py`**

Insert before `checkout_new_branch`:
...
Expected: total **~1290 passed** (1264 + ~26 new from matrix + git_ops); no file >200 lines.
...
inbound message
  ...
  → outbound.sender_agent == agent_name?      [guard 1]
...
```python
if prior_branch:
    "Queued as task #N for AGENT on existing branch `B` (worker pid P)."
elif mutates is False:
    "Queued as task #N for AGENT as read-only (no branch will be created; worker pid P)."
else:
...
def _prior_branch(
    ...
    Returns "" when any link is missing OR when the prior task is in a
    different project OR when the outbound row's sender_agent doesn't
    match agent_name. Defense against misrouted replies inheriting the
    wrong branch."""
    ...
    if not outbound or not outbound.get("task_id"):
        return ""
    if outbound.get("sender_agent") and outbound["sender_agent"] != agent_name:
        logger.info(
            "ignoring prior task: outbound sender_agent=%s != reply agent=%s",
...
def _format_ack(
    *, task_id: int, agent_name: str, worker_pid: int,
    ...
    if prior_branch:
        body_text = (
            f"Queued as task #{task_id} for {agent_name} on existing branch "
            f"`{prior_branch}` (worker pid {worker_pid})."
        )
    elif mutates is False:
        body_text = (
...
Expected: total **~1305 passed** (1290 + ~15 new); `src/reply_router.py` under 200 lines.
...
# Phase G — `enqueue_task_tool` auto-classifies and reports an honest `planned_branch`

One task. Closes reviewer blocker 3: v1 only classified replies, so first-time inbound tasks like "explain the schema" arrived with `mutates_repo=NULL` and still hit the dirty gate. v2 has `enqueue_task_tool` run the classifier when the caller didn't pass an explicit hint.
...
## Task G.11: `enqueue_task_tool` classifies + accurate `planned_branch`
...
Expected: total **~1312 passed** (1305 + 7 new); `chat/project_tools.py` under 200.
...
## Task H.12: Concurrency guard test

The matrix in Phase E assumes serial execution per project — branch reuse breaks if two follow-up tasks try to run on the same branch concurrently. `claim_next` already enforces this via `project_path` + `WHERE status='pending'`, but the invariant is now load-bearing in a way it wasn't before. Pin it explicitly.
...
```python
"""Branch reuse strengthens the 'one running task per project'
invariant. claim_next already filters on project_path so per-project
workers are serial; this test pins that invariant explicitly so any
future change to the queue is caught."""
from src.chat_db import ChatDB
from src.task_queue import TaskQueue
...
Expected: PASS without any code change (current `claim_next` already enforces this; the test exists as a regression guard).

```
.venv/bin/pytest tests/ -q
```

Expected: total **~1315 passed** (1312 + 3 new).
...
Bump the test-count badge if present. New count is whatever `.venv/bin/pytest tests/ -q` reports (≈1315 — confirm before committing).
