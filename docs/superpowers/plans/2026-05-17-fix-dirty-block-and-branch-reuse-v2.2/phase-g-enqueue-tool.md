...
**Blocker: first-time read-only questions must not create branches.** A task created from `chat_enqueue_task` with body `explain the schema` must be stamped `mutates_repo=0`, must return an empty `planned_branch`, and must not create or record a branch when `branch_prep.prepare_branch()` runs.
...
Direct unit tests cover `body=""` for classifier plumbing; real MCP dispatch rejects empty bodies before this tool is called.

If `tests/test_enqueue_task_tool.py` approaches or exceeds the 200-line cap after these additions, split the new mutates_repo/planned_branch tests into `tests/test_enqueue_task_tool_mutation.py`.
...
If the inherited branch no longer exists, `branch_prep` owns the fallback: clean repo creates a fresh branch; dirty repo fails safely.
