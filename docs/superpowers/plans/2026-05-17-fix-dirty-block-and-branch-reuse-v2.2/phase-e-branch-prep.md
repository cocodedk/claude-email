...
class TestPriorBranchAlreadyOn:
    ...
    def test_runs_even_if_dirty(self, queue, mocker, mutates):
        ...
        # is_clean must NOT have been called — the on-prior shortcut
        # returns True before reaching it. A refactor that adds an
        # is_clean call here would silently change the contract.
        is_clean.assert_not_called()
        queue.mark_failed.assert_not_called()
