# Consolidated v2.2 Plan — Fix Dirty-Block & Branch Reuse

> **Flattened view of the 9-file plan in `2026-05-17-fix-dirty-block-and-branch-reuse-v2.2/`.**
> This flattened file is for review. When executing, copy the approved changes back into the phase files first.

**Version:** v2.2 (2026-05-17 — round-3 + round-4 + round-5 reviewer fixes folded into the v2.2 folder)
**Review history:** see the "Revision 2 (round-3 review)" section in README.md below — 6 round-3 blockers + 3 non-blockers + the round-4 corrections (status, line-cap, ACK wording, retry fallback, dispatch note, etc.) + the round-5 fixes (classifier punctuation tolerance, Phase F prereq narrative, tightened on-prior test, consolidated-file version refresh) — all patched across phases A, B, E, F, G, H. v1 and v2 are archived under `../archive/`.
...
6. **Blocker: first-time read-only questions must not create branches.** v2 classified first-time tasks and returned an empty `planned_branch`, but it did not name the user-facing regression explicitly or pin the full path. Phase G.11a now adds a regression that queues `explain the schema`, verifies `mutates_repo=0`, runs `branch_prep.prepare_branch`, and asserts no dirty check, no `checkout_new_branch`, no `set_branch`, and `task.branch_name IS NULL`.
...
- **G.11a** `enqueue_task_tool` auto-classifies + accurate `planned_branch`. Closes v2 reviewer blocker 3 and the named blocker: first-time read-only questions must not create branches.
...
- Modify: `tests/test_enqueue_task_tool.py` (add classifier / planned-branch / first-time no-branch regressions)
...
class TestFirstTimeReadOnlyDoesNotCreateBranch:
    """Named blocker: first-time read-only questions must not create
    branches. This pins the full path, not just the tool response:
    enqueue_task_tool stamps mutates_repo=0, then branch_prep skips the
    dirty check, skips checkout_new_branch, and leaves branch_name NULL."""

    def test_first_time_read_only_question_never_branches(
        self, tq, mgr, tmp_path, mocker,
    ):
        ...
        set_branch = mocker.spy(tq, "set_branch")

        assert branch_prep.prepare_branch(
            tq, tq.get(result["task_id"]), str((tmp_path / "p").resolve()),
        ) is True
        is_clean.assert_not_called()
...
- Modify: `tests/test_enqueue_task_tool.py` (`TestHighPriorityJumpsQueue` must stop expecting a second simultaneous claim)
...
- [ ] **Step 4: Update the existing high-priority queue test**

Current `tests/test_enqueue_task_tool.py::TestHighPriorityJumpsQueue::test_higher_priority_claimed_first` says:
...
That comment and assertion are now wrong. Replace the tail with:

```python
# claim_next now enforces one running task per project. Finish the
# high-priority task before claiming the next one.
assert tq.claim_next(str((tmp_path / "p").resolve())) is None
tq.mark_done(first["id"])
second = tq.claim_next(str((tmp_path / "p").resolve()))
assert second["id"] == low["task_id"]
```

This preserves the ordering check while accepting the new one-running-task-per-project invariant.

- [ ] **Step 5: Run all tests**

```
.venv/bin/pytest tests/test_one_running_per_branch.py tests/test_task_queue.py tests/test_enqueue_task_tool.py -v
.venv/bin/pytest tests/ -q
```
...
```bash
git add src/task_queue.py tests/test_one_running_per_branch.py tests/test_enqueue_task_tool.py
...
require a clean working tree and no longer fork a per-task branch. This
applies to first-time questions as well as follow-up replies.
