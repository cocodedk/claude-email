...
Three tasks. H.12 changes production code by adding the queue-level one-running-task-per-project guard. H.13 updates docs. H.14 runs simplify, coverage, migration smoke, and final verification.
...
Existing test update: `tests/test_enqueue_task_tool.py::TestHighPriorityJumpsQueue::test_higher_priority_claimed_first` currently asserts the old permissive queue behavior by claiming the low-priority task while the high-priority task is still running. After the `NOT EXISTS running` guard, that second claim must return `None` until the first task is marked done.
...
Make sure the duplicate earlier `first = ...` line is not left in the test twice. This preserves the ordering check while accepting the new one-running-task-per-project invariant.
...
- [ ] Strict 200-line
- [ ] 100% coverage
- [ ] Test file line cap. Run `scripts/check-line-limit.sh` after adding the new regression tests. If any test file exceeds 200 lines, split by behavior, not by phase.
