...
**Prerequisites:** Phases A.2, A.3, and C.6 must be applied before this task. F.10's test fixtures call `db.record_outbound_email(..., task_id=...)` (needs A.3), do raw INSERTs into `outbound_emails(..., task_id, ...)` (needs A.2's schema column), and pass `branch_name=` / `mutates_repo=` to `tq.enqueue(...)` (needs C.6). If you run F before A or C, every test in this phase fails at fixture setup with `TypeError` or `sqlite3.OperationalError` rather than at the `apply_reply` call site — chase the prerequisite, not the apply_reply signature.
...
Expected: FAIL — `apply_reply()` has no `original_email_message_id` kwarg. (If you see `TypeError: record_outbound_email() got an unexpected keyword argument 'task_id'` or `sqlite3.OperationalError: no column named task_id` instead, Phase A.2 / A.3 hasn't been applied yet — go back and finish Phase A first.)
